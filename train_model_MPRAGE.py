...
batch_factors = np.linspace(3, 3, 1).astype(int).tolist()  # 3
do_weights = [False]
model_names = ["DBN_InceptionResnetv2.h5"]  #
losses = [mean_squared_error]
learning_rates = (7 * np.logspace(-5, -4, 1)).tolist()  # 3
moderates = [False]
formulas = [
    "brainage ~ age",
    # "brainage ~ age * scanner",
]
correction_method = "clear"  # cov
do_wls = [False]
layers_to_train = 3
ow = True
useaic = True  # deprecated
clone_last_layers = True
...
variable_file = os.path.join(variables_folder, f"cv_grid_setup_MPRAGE.pkl")
...
# Remove all modalities but original MPRAGE
modalities = ["MPRAGE"]
for df in (
    train_df_list
    + linear_df_list
    + valid_df_list
    + [final_train_df, final_linear_df, final_eval_df, test_df]
):
    df.drop(df[df["modality"] != "MPRAGE"].index, inplace=True)
                    ...
                    for loss, learning_rate, moderate in product(
                        ...
                        with strategy.scope():
                            ...
                            model = create_model(
                                ...
                                len(scanners),
                                moderate,
                                clone_last_layers=clone_last_layers,
                            )
...
now = datetime.datetime.now()
date_string = now.strftime("%Y-%m-%d_%H-%M-%S")
variable_file = os.path.join(variables_folder, f"variables_after_cv_MPRAGE.pkl")
with open(variable_file, "wb") as f:
    pickle.dump(
        [
            mae_valid_array,
...
final_corrected_model_aic.save(os.path.join(results_folder, "new_aic_MPRAGE_" + filename))
final_corrected_model_aic_corr.save(
    os.path.join(results_folder, "new_aic_corr_MPRAGE_" + filename)
)
final_corrected_model.save(os.path.join(results_folder, "new_MPRAGE_" + filename))
model.save(os.path.join(results_folder, "trained_MPRAGE_" + filename))

# Save all local variables
now = datetime.datetime.now()
date_string = now.strftime("%Y-%m-%d_%H-%M-%S")
variable_file = os.path.join(variables_folder, f"variables_final_model_MPRAGE.pkl")
with open(variable_file, "wb") as f:
    pickle.dump(
        [
            final_flow_args,
